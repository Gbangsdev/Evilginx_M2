...
go_version_ok() {
    ...
    # Accept Go 1.22+ (matches go.mod requirement)
    if [[ "$ver" =~ ^1\.(2[2-9]|[3-9][0-9]) ]]; then
        return 0
    fi
    return 1
}
...
install_go() {
    ...
    log_info "Detected CPU architecture: $(uname -m) -> go linux-${GO_ARCH}"

    # Keep a working Go 1.22+ if already on PATH (e.g. user-installed arm64 tarball)
    if go_version_ok; then
        log_success "Compatible Go already installed: $(go version)"
        return 0
    fi

    # Remove broken / wrong-architecture install
    if [ -d /usr/local/go ]; then
        log_info "Removing existing /usr/local/go"
        rm -rf /usr/local/go
    fi
    ...
    # Prefer tarball in installing user's home, then script dir, then /root
    for candidate in "$HOME/$GO_TARBALL" "$SCRIPT_DIR/$GO_TARBALL" "/root/$GO_TARBALL"; do
        if [[ -f "$candidate" ]]; then
    ...
    if [[ -n "$LOCAL_TARBALL" ]]; then
        log_info "Using local Go tarball: $LOCAL_TARBALL"
        cp "$LOCAL_TARBALL" "/tmp/$GO_TARBALL"
    else
        log_info "Downloading Go $GO_VERSION for linux-${GO_ARCH}..."
    ...
    # Verify binary matches this machine before continuing
    if ! /usr/local/go/bin/go version >/dev/null 2>&1; then
        log_error "Installed Go binary cannot run on this system (wrong architecture?)"
        log_error "Expected: linux-${GO_ARCH}, got Exec format error"
        rm -rf /usr/local/go
        exit 1
    fi
    ...
    log_success "Go installed successfully: $(/usr/local/go/bin/go version)"
